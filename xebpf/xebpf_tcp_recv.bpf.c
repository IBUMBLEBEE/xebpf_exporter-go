...
int BPF_PROG(tcp_recvmsg_fn, struct sock *sk, struct msghdr *msg, size_t len, int nonblock,
    ...
    if (check_ports(bpf_ntohs(dst_port))) {
        ...
        val = bpf_map_lookup_elem(&xebpf_tcp_recv_bytes, &key);
        if (val) {
            __sync_fetch_and_add(val, len);
        } else {
            __u64 val_new = len;
            bpf_map_update_elem(&xebpf_tcp_recv_bytes, &key, &val_new, BPF_ANY);
        }
    }
    return 0;
}